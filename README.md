...
Развёртывание всей инфраструктуры (включая СУБД, брокер RabbitMQ и каждый микросервис отдельно) осуществляется из подготовленных манифестов в пространстве имён `homework`:
...
3. **Проверка отправленных писем (нотификаций):**
   Для проверки выполнения сценариев тестов Postman и просмотра сохраненных микросервисом сообщений используется выделенный HTTP-эндпоинт сервиса уведомлений: 
